...
public class PatientLinkageGadget<T> extends Gadget<T> {
	...
	public static int[] getIntArrayFromStrs(String str) {
		...
		final int[] retArr = new int[strs.length];
	...
	public Object secureCompute(CompEnv<T> e, Object[] o) throws Exception {
		...
		final T[][][] a = (T[][][]) ((PatientLinkage4GadgetInputs) o[0]).getInputs();
		final T[][][] b = (T[][][]) ((PatientLinkage4GadgetInputs) o[1]).getInputs();
		final int rows = a.length;
		final int cols = b.length;
		final int numComps = a[0].length;
		final T[][] ret = e.newTArray(rows, cols);
		final IntegerLib<T> int_lib = new IntegerLib<>(e);
