...
public class TestFilterFuncs {
	...
	static int[] T = new int[] { 98, 6, 85, 150, 36, 23, 112, 164, 135, 207, 169, 5, 26, 64, 165, 219, 61, 20, 68, 89, 130, 63, 52, 102, 24, 229, 132, 245, 80, 216, 195, 115, 90, 168, 156, 203, 177, 120, 2, 190, 188, 7, 100, 185, 174, 243, 162, 10, 237, 18, 253, 225, 8, 208, 172, 244, 255, 126, 101,
			79, 145, 235, 228, 121, 123, 251, 67, 250, 161, 0, 107, 97, 241, 111, 181, 82, 249, 33, 69, 55, 59, 153, 29, 9, 213, 167, 84, 93, 30, 46, 94, 75, 151, 114, 73, 222, 197, 96, 210, 45, 16, 227, 248, 202, 51, 152, 252, 125, 81, 206, 215, 186, 39, 158, 178, 187, 131, 136, 1, 49, 50, 17, 141,
			91, 47, 129, 60, 99, 154, 35, 86, 171, 105, 34, 38, 200, 147, 58, 77, 118, 173, 246, 76, 254, 133, 232, 196, 144, 198, 124, 53, 4, 108, 74, 223, 234, 134, 230, 157, 139, 189, 205, 199, 128, 176, 19, 211, 236, 127, 192, 231, 70, 233, 88, 146, 44, 183, 201, 22, 83, 13, 214, 116, 109, 159,
			32, 95, 226, 140, 220, 57, 12, 221, 31, 209, 182, 143, 92, 149, 184, 148, 62, 113, 65, 37, 27, 106, 166, 3, 14, 204, 72, 21, 41, 56, 66, 28, 193, 40, 217, 25, 54, 179, 117, 238, 87, 240, 155, 180, 170, 242, 212, 191, 163, 78, 218, 137, 194, 175, 110, 43, 119, 224, 71, 122, 142, 42, 160,
			104, 48, 247, 103, 15, 11, 138, 239 };

	public static byte calSum(byte[] arr) {

		int ret = 0;
		for (final byte arr1 : arr) {
			ret += arr1;
		}
		return (byte) (ret % 256);
	}
	...
	public static String[][] combineProps(String[][] props) {
		...
		final String[][] com_strs = new String[props.length][4];
	...
	public static byte[] convertBoolean2Bytes(boolean[] bl_arr) {
		...
		final int byte_len = (bl_arr.length + 7) / 8;
		final byte[] arrs = new byte[byte_len];
		final boolean[] bl_arr_src = new boolean[byte_len * 8];
		System.arraycopy(bl_arr, 0, bl_arr_src, 0, bl_arr.length);
		final boolean[] bl_tmp = new boolean[8];
	...
	public static int getPatientLinkageNumByHash_1_byte(String[][] com_strs1, String[][] com_strs2, int len, int alg) {
		...
		int num = 0;
		boolean flag;
		for (final byte[][] com_strs11 : b_com_strs1) {
			for (final byte[][] com_strs21 : b_com_strs2) {
				flag = false;
	...
	public static byte[][][] getStringHashSum(String[][] strs) {
		...
		final byte[][][] ret_arr = new byte[strs.length][strs[0].length][1];
	...
	public static void main(String[] args) {
		...
		final String file1 = "/Users/cf/GitHub/patientlinkage/data/Source14k_a_10K.csv";
		final String file2 = "/Users/cf/GitHub/patientlinkage/data/Source14k_b_10K.csv";
		final int[] lens = new int[] { 0, 12, 11, 0, 0, 0, 9, 0, 0, 0, 0, 8 };
		final String[][] data1 = readCSVFile(file1, lens);
		final String[][] data2 = readCSVFile(file2, lens);
		final String[][] com_strs1 = combineProps(data1);
		final String[][] com_strs2 = combineProps(data2);
	...
	public static byte Pearson8Bit(String str) {
		...
		final byte[] byte_arr = str.getBytes(StandardCharsets.UTF_8);
		int h = T[byte_arr[0] % 256];
		for (final byte element : byte_arr) {
			h = T[(h ^ element) % 256];
		}
		return (byte) h;
	}
	...
	public static String[][] readCSVFile(String FileName, int[] lens) {
		...
		final ArrayList<String[]> retArrList = new ArrayList<>();
		int prop_num = 0;
		final boolean[] mask = new boolean[lens.length];
		...
		try (CSVReader reader = new CSVReader(new FileReader(FileName))) {
			...
			while ((strs = reader.readNext()) != null) {
				final String[] coms_strs = new String[prop_num];
				int index = 0;
		...
		final String[][] ret_str = new String[retArrList.size()][];
	...
	public static byte[][][] toPearson8Bit(String[][] com_strs) {
		...
		final byte[][][] hash_bytes_arr = new byte[com_strs.length][com_strs[0].length][1];
		for (int n = 0; n < com_strs.length; n++) {
			for (int k = 0; k < com_strs[n].length; k++) {
